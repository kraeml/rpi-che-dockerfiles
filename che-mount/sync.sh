...
Usage on Linux 
  docker run --rm -it --cap-add SYS_ADMIN --device /dev/fuse
            ...
            -u \$(id -u \${USER})
            -v <local-mount>/:/mnthost kraeml/rpi-che-mount <workspace-id|workspace-name>
...
docker run --rm  -v /var/run/docker.sock:/var/run/docker.sock kraeml/rpi-che-action:${CHE_VERSION} get-ssh-data ${WORKSPACE_NAME} ${COMMAND_EXTRA_ARGS} > $HOME/env
if [ $? -ne 0 ]; then
    error "ERROR: Error when trying to get workspace data for workspace named ${WORKSPACE_NAME}"
    echo "List of workspaces are:"
    docker run --rm  -v /var/run/docker.sock:/var/run/docker.sock kraeml/rpi-che-action:${CHE_VERSION} list-workspaces
    return 1
fi
